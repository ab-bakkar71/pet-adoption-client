...
const MyListingClient = ({ deletePet }) => {
    ...
    const handlePetDeleted = (deletedId) => {
        setMyPets((prev) => prev.filter((p) => p._id !== deletedId));
    };
                            ...
                            {myPets.length === 0 ? (
                                <div className="h-[50vh] flex flex-col items-center justify-center text-center">
                                    ...
                                    <p className="text-slate-400 mt-2 text-sm max-w-sm">
                                        Give a homeless animal a chance. Add your first pet for adoption today.
                                    </p>
                                    <Link
                                        href="/dashboard/adopt"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                                    {myPets.map((pet) => (
                                        <div
                                            key={pet._id}
                                            className="p-4 bg-white dark:border dark:bg-slate-900/90 dark:border-slate-800/80 hover:-translate-y-1 transition duration-300 rounded-lg shadow-sm flex flex-col justify-between"
                                        >
                                            <div>
                                                {/* Image Container */}
