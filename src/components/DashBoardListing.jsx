...
const DashBoardListing = () => {
    ...
    return (
        <div className="lg:col-span-3 space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 text-center gap-4">
                <div className="rounded-2xl border bg-white border-slate-200 shadow-sm dark:bg-slate-900/40 dark:border-slate-800/80 overflow-hidden p-5">
                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Pets</h3>
                    <p className="text-2xl font-bold text-slate-800 dark:text-white mt-1">{myPets.length}</p>
                </div>
                <div className="rounded-2xl border bg-white border-slate-200 shadow-sm dark:bg-slate-900/40 dark:border-slate-800/80 overflow-hidden p-5">
                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Available</h3>
                    <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">
                        {myPets.filter(pet => pet.status?.toLowerCase() === 'available').length}
                    </p>
                </div>
                <div className="rounded-2xl border bg-white border-slate-200 shadow-sm dark:bg-slate-900/40 dark:border-slate-800/80 overflow-hidden p-5">
                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Adopted</h3>
                    <p className="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">
                        {myPets.filter(pet => pet.status?.toLowerCase() === 'adopted').length}
                    </p>
                </div>
                <div className="rounded-2xl border bg-white border-slate-200 shadow-sm dark:bg-slate-900/40 dark:border-slate-800/80 overflow-hidden p-5">
                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pending</h3>
                    <p className="text-2xl font-bold text-amber-600 dark:text-amber-400 mt-1">
                        {myPets.filter(pet => pet.status?.toLowerCase() === 'pending').length}
                    </p>
                </div>
                <div className="rounded-2xl border bg-white border-slate-200 shadow-sm dark:bg-slate-900/40 dark:border-slate-800/80 overflow-hidden p-5">
                    <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Cancelled</h3>
                    <p className="text-2xl font-bold text-rose-600 dark:text-rose-400 mt-1">
                        {myPets.filter(pet => pet.status?.toLowerCase() === 'cancel' || pet.status?.toLowerCase() === 'cancelled').length}
                    </p>
                </div>
            </div>
                            ...
                            {myPets.length === 0 ? (
                                <tr>
                                    <td colSpan={5} className="p-8 text-center text-slate-400 dark:text-slate-500">
                                        {loading ? "Loading your pet listings..." : "No pets listed yet. Click 'List a New Pet' to get started."}
                                    </td>
                                </tr>
                            ) : (
                                myPets.map((pet) => (
                                    <tr key={pet._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20 transition-colors">
                                        ...
                                        <td className="p-4 text-right">
                                            <Link
                                                ...
                                                className="text-xs font-bold px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-600 dark:border-slate-800 dark:hover:bg-slate-900 dark:text-slate-400 transition-colors inline-block"
                                            >
                                                Manage
                                            </Link>
                                        </td>
